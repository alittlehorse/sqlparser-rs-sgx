...
static sgx_status_t SGX_CDECL sgx_lexer(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_lexer_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_lexer_t* ms = SGX_CAST(ms_lexer_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	const uint8_t* _tmp_sql = ms->ms_sql;
	...
	ms->ms_retval = lexer((const uint8_t*)_in_sql, _tmp_len);
